...
4. Test the installation by runing the following command in the VS Code terminal or Anaconda Prompt:
   ```powershell 
   python -c "from rdkit import Chem; mol = Chem.MolFromSmiles('C1CCCCC1'); print(mol)"
   ```
    Expected output:
   ```
   ...
   ```
    ...
    ```powershell 
    python \src\features\substrate_emb_ChamBERTA2.py
    ```
   
    VISUALIZATION: 
    ```powershell 
    python .\src\utils\visualize_substrate_embeddings.py
    ```
    ...
    ```python
    .\scripts\analyze_cluster_properties.py
    ```
---
...
To validate our model selection, we compared multiple approaches from baseline classifiers to advanced deep learning architectures:

**Performance Comparison on C2 Test Set (Novel GT or Substrate - Primary Metric):**

| Model | C2 F1 Score | C2 Accuracy | C2 ROC-AUC | Improvement vs Baseline |
|-------|------------|-------------|------------|-------------------------|
...
| Model | C1 F1 | C1 Accuracy | C2 F1 | C2 Accuracy | C3 F1 | C3 Accuracy | Average F1 |
|-------|-------|-------------|-------|-------------|-------|-------------|------------|
...
The **Attention MLP architecture** was selected based on:
1. **Superior C2 generalization** (F1=0.732±0.026) - the primary use case for predicting novel GT-substrate interactions
2. **Balanced performance** across all test sets without sacrificing generalization for memorization
