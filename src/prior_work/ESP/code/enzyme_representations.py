...
def calcualte_esm1b_vectors(enzyme_list):
	...
	model, alphabet = esm.pretrained.load_model_and_alphabet_local(model_location = Path(__file__).parent.parent / "data" / "saved_models" /
...
def calcualte_esm1b_ts_vectors(enzyme_list):
	...
	model_location = Path(__file__).parent.parent / "data" / "saved_models" / "ESM1b" / "esm1b_t33_650M_UR50S.pt"
	
	model_data = torch.load(model_location, map_location='cpu', weights_only=False)
	regression_location = model_location.parent / "esm1b_t33_650M_UR50S-contact-regression.pt"
	...
	model.eval()
	PATH = Path(__file__).parent.parent / "data" / "saved_models" / "ESM1b" / 'model_ESM_binary_A100_epoch_1_new_split.pkl'
