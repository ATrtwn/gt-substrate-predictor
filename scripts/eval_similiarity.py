...
def get_percentage_similarities_fast(df, threshold=90, min_coverage=0.8):
    ...
    best_matches = df.loc[df.groupby('query')['pident'].idxmax()]
    best_matches = best_matches.set_index('query')[['pident', 'target', 'qcov', 'tcov', 'evalue']]
    ...
    result = pd.DataFrame({
        ...
        'pident_max': best_matches['pident'],
        'target_best': best_matches['target'],
        'qcov': best_matches['qcov'],
        'tcov': best_matches['tcov'],
        'evalue': best_matches['evalue']
    }).reset_index()
...
def calculate_leakage_stats(max_sim_df, thresholds=[30, 50, 70, 80, 90, 95, 100]):
    ...
    for thr in thresholds:
        above = len(max_sim_df[max_sim_df['pident'] >= thr])
        stats[thr] = {
            'count': above,
...
test_c1_max = get_max_similarities(test_c1_df, min_coverage=0.8)
test_c2_max = get_max_similarities(test_c2_df, min_coverage=0.8)
val_c1_max = get_max_similarities(val_c1_df, min_coverage=0.8)
val_c2_max = get_max_similarities(val_c2_df, min_coverage=0.8)

# Calculate for all datasets
test_c1_stats = calculate_leakage_stats(test_c1_max)
test_c2_stats = calculate_leakage_stats(test_c2_max)
val_c1_stats = calculate_leakage_stats(val_c1_max)
val_c2_stats = calculate_leakage_stats(val_c2_max)

# Print results
print("C1 Leakage Analysis:")
print("=" * 50)
for thr, data in test_c1_stats.items():
    print(f">={thr}% identity: {data['count']}/{data['total']} ({data['percentage']:.2f}%)")
...
print("\nValC1 Leakage Analysis:")
print("=" * 50)
for thr, data in val_c1_stats.items():
    print(f">={thr}% identity: {data['count']}/{data['total']} ({data['percentage']:.2f}%)")
print("\nValC2 Leakage Analysis:")
print("=" * 50)
for thr, data in val_c2_stats.items():
    print(f">={thr}% identity: {data['count']}/{data['total']} ({data['percentage']:.2f}%)")

for stat_name, stat_func in [("Mean Similarity", mean_similarity),
                             (">80% Identity", prop_above_80),
                             (">95% Identity", prop_above_95)]:
    
    result = bootstrap_statistic(test_c1_max['pident'].values, stat_func, n_bootstrap=1000)
...
# 1. Distribution of maximum similarities
axes[0, 0].hist(test_c1_max['pident'], bins=50, alpha=0.5, label='C1', density=True)
axes[0, 0].hist(test_c2_max['pident'], bins=50, alpha=0.5, label='C2', density=True)
axes[0, 0].hist(val_c1_max['pident'], bins=50, alpha=0.5, label='Val C1', density=True)
axes[0, 0].hist(val_c2_max['pident'], bins=50, alpha=0.5, label='Val C2', density=True)

axes[0, 0].set_xlabel('Maximum Percent Identity to Train')
axes[0, 0].set_ylabel('Density')
axes[0, 0].set_title('Distribution of Maximum Similarities')
axes[0, 0].legend()
...
sorted_test_c1 = np.sort(test_c1_max['pident'])
sorted_test_c2 = np.sort(test_c2_max['pident'])
sorted_val_c1 = np.sort(val_c1_max['pident'])
sorted_val_c2 = np.sort(val_c2_max['pident'])
...
axes[0, 1].set_xlabel('Percent Identity')
...
c1_leakage = [np.mean(test_c1_max['pident'] >= t) * 100 for t in thresholds]
c2_leakage = [np.mean(test_c2_max['pident'] >= t) * 100 for t in thresholds]
val_c1_leakage = [np.mean(val_c1_max['pident'] >= t) * 100 for t in thresholds]
val_c2_leakage = [np.mean(val_c2_max['pident'] >= t) * 100 for t in thresholds]
...
axes[0, 2].set_xlabel('Identity Threshold (%)')
axes[0, 2].set_ylabel('% of Test Sequences')
axes[0, 2].set_title('Leakage by Threshold')
axes[0, 2].legend()
axes[0, 2].grid(True, alpha=0.3)

# 4. Bootstrap distribution for mean similarity
c1_mean_boot = bootstrap_statistic(test_c1_max['pident'].values, mean_similarity, n_bootstrap=1000)
c2_mean_boot = bootstrap_statistic(test_c2_max['pident'].values, mean_similarity, n_bootstrap=1000)
val_c1_mean_boot = bootstrap_statistic(val_c1_max['pident'].values, mean_similarity, n_bootstrap=1000)
val_c2_mean_boot = bootstrap_statistic(val_c2_max['pident'].values, mean_similarity, n_bootstrap=1000)
...
axes[1, 0].set_title('Bootstrap Distribution of Mean Similarity')
axes[1, 0].legend()
...
axes[1, 1].set_ylabel('Mean Similarity (%)')
axes[1, 1].set_title('Mean Similarity with 95% CI')
...
summary_text = [
    "SUMMARY STATISTICS",
    "=" * 30,
    f"C1 Test Set (n={len(test_c1_max)})",
    f"  Mean similarity: {np.mean(test_c1_max['pident']):.1f}%",
    f"  Median similarity: {np.median(test_c1_max['pident']):.1f}%",
    f"  >80% identical: {np.mean(test_c1_max['pident'] >= 80)*100:.1f}%",
    f"  >95% identical: {np.mean(test_c1_max['pident'] >= 95)*100:.1f}%",
    "",
    f"C2 Test Set (n={len(test_c2_max)})",
    f"  Mean similarity: {np.mean(test_c2_max['pident']):.1f}%",
    f"  Median similarity: {np.median(test_c2_max['pident']):.1f}%",
    f"  >80% identical: {np.mean(test_c2_max['pident'] >= 80)*100:.1f}%",
    f"  >95% identical: {np.mean(test_c2_max['pident'] >= 95)*100:.1f}%",
    "",
    f"Val C1 Set (n={len(val_c1_max)})",
    f"  Mean similarity: {np.mean(val_c1_max['pident']):.1f}%",
    f"  Median similarity: {np.median(val_c1_max['pident']):.1f}%",
    f"  >80% identical: {np.mean(val_c1_max['pident'] >= 80)*100:.1f}%",
    f"  >95% identical: {np.mean(val_c1_max['pident'] >= 95)*100:.1f}%",
    "",
    f"Val C2 Set (n={len(val_c2_max)})",
    f"  Mean similarity: {np.mean(val_c2_max['pident']):.1f}%",
    f"  Median similarity: {np.median(val_c2_max['pident']):.1f}%",
    f"  >80% identical: {np.mean(val_c2_max['pident'] >= 80)*100:.1f}%",
    f"  >95% identical: {np.mean(val_c2_max['pident'] >= 95)*100:.1f}%"
]
...
def generate_report(test_c1_max, test_c2_max, val_c1_max, val_c2_max, filename='leakage_report.txt'):
    """Generate a comprehensive report"""
    
    with open(filename, 'w') as f:
        f.write("DATA LEAKAGE ANALYSIS REPORT\n")
        f.write("=" * 50 + "\n\n")
        
        f.write("1. DATASET OVERVIEW\n")
        f.write("-" * 30 + "\n")
        f.write(f"C1 test sequences: {len(test_c1_max)}\n")
        f.write(f"C2 test sequences: {len(test_c2_max)}\n\n")
        f.write(f"Val C1 sequences: {len(val_c1_max)}\n")
        f.write(f"Val C2 sequences: {len(val_c2_max)}\n\n")

        f.write("2. SIMILARITY STATISTICS\n")
        f.write("-" * 30 + "\n")
        
        for name, data in [("C1", test_c1_max), ("C2", test_c2_max),
                           ("Val C1", val_c1_max), ("Val C2", val_c2_max)]:
            f.write(f"\n{name}:\n")
            f.write(f"  Mean ± SE: {np.mean(data['pident']):.2f}% ± {np.std(data['pident'])/np.sqrt(len(data)):.3f}%\n")
            f.write(f"  Median: {np.median(data['pident']):.2f}%\n")
            f.write(f"  Range: [{data['pident'].min():.2f}%, {data['pident'].max():.2f}%]\n")
            ...
            for stat_name, stat_func, thr in [("Mean", mean_similarity, None),
                ...
                result = bootstrap_statistic(data['pident'].values, stat_func, n_bootstrap=1000)
        ...
        for name, data in [("C1", test_c1_max), ("C2", test_c2_max),
                           ("Val C1", val_c1_max), ("Val C2", val_c2_max)]:
            prop_95 = np.mean(data['pident'] >= 95) * 100
            ...
            f.write(f"\n{name}: {assessment}\n")
            f.write(f"  Rationale: {prop_95:.2f}% of test sequences are >95% identical to training\n")
        
        f.write("\n4. RECOMMENDATIONS\n")
        f.write("-" * 30 + "\n")
        f.write("1. For sequences with >95% identity to training: Remove from test set\n")
        f.write("2. For sequences with 80-95% identity: Consider if this level of similarity\n")
...
# Generate the report
generate_report(test_c1_max, test_c2_max, val_c1_max, val_c2_max)
print("Report saved as 'leakage_report.txt'")
